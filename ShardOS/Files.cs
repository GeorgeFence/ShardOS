...
namespace ShardOS
{
    public static class Files
    {
        ...
        public static void Initialize()
        {
            try
            {
                Kernel.DrawStatus("Initializing FAT system");
                Driver = new CosmosVFS();
                Sys.FileSystem.VFS.VFSManager.RegisterVFS(Driver);
                Kernel.DrawStatus("FAT System Initialized");
            }
            // unknown exception
            catch (Exception ex)
            {
                Kernel.DrawStatus("Cannot initialize FAT system: " + ex.Message + ", Rebooting now", Color.Red);
